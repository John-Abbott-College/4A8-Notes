...
* Users can edit values directly in the `DataGrid`, which updates the underlying data source

  

  > You cannot format a column if `AutoGenerateColumns` is `true`
  >
  > When `AutoGenerateColumns = false`  you define each column manually, which allows full formatting and control
...
//the whole table is a collection of rows
//Dictionary<string, object> is one row
...
foreach (string key in populations[0].Keys)
{
    ...
    column.Header = key;
    column.Binding = new Binding($"[{key}]"); // Notice the square brackets! gets value for {key}
...
When you attach a `ContextMenu` to a `DataGrid`:

- The menu appears when the user **right-clicks anywhere on the grid**
- It is typically used to perform actions on the **data behind the grid rows**
