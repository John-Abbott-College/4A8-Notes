...
| File                                 | Description                                                  |
| :----------------------------------- | :----------------------------------------------------------- |
| `CICD_Practice.sln`                  | This is the **solution fil**e used by Visual Studio which describes the various necessities to create our projects |
| `CICD_Practice/App.xaml`             | Main application configuration. Defines things that apply to the **entire application** (examples: which window starts first, shared styles) |
...
| `CICD_Practice/MainWindow.xaml`      | Defines the UI layout of the main window.                    |
| `CICD_Practice/MainWindow.xaml.cs`   | Contains the code-behind (logic) for the main window.        |
| `CICD_Practice/dll/*`                | Stores external libraries (e.g., the HomeBudget DLL used in this project). |
| `Test/Test.csproj`                   | Project file for the test project.                           |
| `Test/UnitTest1.cs`                  | Contains unit tests written using xUnit.                     |
...
1. Go to your home directory first... easiest way is by using the environment variable `USERPROFILE`
   ...
   * The Windows command line interface (CLI) uses backslashes (`\`) for directory paths, while Unix-based systems (like bash) use forward slashes (`/`).
   * To autocomplete file names, press tab, if you don't see what you want, hit tab again.
   * In `bash`, variables are accessed by using the dollar sign `$` in front of the variable name, but `CLI` does it different.  
     * Enclose the variable name in percent signs `%`.  Example: `%variable%`.
   * To see which environment variables are currently available, type `set` and hit return

2. Navigate to the directory that contains the solutions file.

   1. `dir` lists the contents of a directory. It is equivalent to `ls` in bash.
...
* We know that Visual Studio uses it to build. It must be included with the VS application. 

* A good place to look for `msbuild.exe` is where VS 2022 is installed. It is normally in  `C:\Program Files` for 64 bit applications or `C:\Program Files (x86)`.

  ℹ️

  * To list a specific file, use `dir` *`filename`*
  * If you are looking for files, but you don't know which sub-directory it is in, you can use the switch `/s` 
    * BUT: you can only search through sub-directories below the one you are currently in
  * like `bash`, you can use `pushd` *`directory`* which pushes the current directory onto a stack, and then changes to the specified directory.  `popd` will pop off a directory from the stack, and change to that directory

So, for me, VS 2022 Enterprise in `C:\Program Files`  We are going to save our current directory in a stack by using `pushd`.
...
Again, since I am lazy, and do not need this path to be permanent, I am going to set the path in the CLI window, and it will *survive* as long as I have the `cmd` window open.

ℹ️

* To set an environment variable: `set name=value`

⚠️ Remember, when setting the PATH environment variable, you also need to keep what was there before.  Path names are separated by semi-colons `;`.  Because there is a space in the path name, it needs to be enclosed in quotes.
...
1. Go back to the directory where the solution file is
...
2. Now try `msbuild `*`solution_file`*
...
Reading the error message carefully, it mentions that we are missing a `Nuget` package.  Our project uses packages, so I guess that's the problem.
...
If the `dotnet` CLI is not in your path, you must find it and add it.  

1. Repeat the process that we used to find `msbuild.exe`.

2. Repeat the process that we used to add `msbuild.exe`.  

   >  From a Microsoft documentation search, it seems that `dotnet` is installed directly in `C:\Program Files\`
