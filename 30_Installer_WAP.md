...
An installer is an application that

1. Takes care of installing all the pieces needed for your application to run for your users.
2. Allows the user to configure some details of the installation.



## Information you need for later

### Determining which architecture your application is built for and can run on (x32, x64)
...
Copy the `.exe` file. Go to a new folder (on your Desktop, or any other folder outside of your solution). Paste the copy of the .exe file there. 
...
* generic `dll`s, such as `[yourAPI].dll`, `sqlite.dll`
...
* an executable that acts as a bootstrapper (prepares everything needed before the main program can run or install correctly). It verifies that the the software program can be installed on the computer and then allows the user to interact with the installer. Internally it uses (one or more) `*.msi` file(s).
...
# Creating an `MSIX` Installer for your application
...
In Visual Assets, you will be able to see the Icons for your application. **More on this next week.**
...
MSIX allows you to publish your desktop app to the Microsoft Store. We will not be doing this.

Instead, we will be using the option to create a local installer instead. To distribute your application you could give any 'customer'  the installer.
