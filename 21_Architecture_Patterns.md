...
An **interface** is a *contract* that states that any class that *implements* this **interface** will contain all the necessary methods and properties. It defines *what a class must be able to do*, without saying *how it does it*.
...
`BestPizza`, an instance of `Restaurant`, requires a `FoodTakeOutInterface`.
...
* So it needs a `class` that uses the `FoodTakeOutInterface`.
...
```csharp
public Main () {
  TheCommercialSpace building = new TheCommerialSpace(); // TheCommercialSpace implements the interface
...
* Other than query about *state* the **model** does not make any decisions
...
* The **view** is what the user *sees* and interacts with.
  ...
  *   in MVVM
    * the **view**'s data is *bound* to the data controlled and manipulated by the **view model**
...
- The View captures user events, and triggers the ViewModel commands or updates properties, which in turn updates the Model and bound data.
