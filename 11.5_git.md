...
If you accidentally commit a folder before adding `.gitignore`, run:
...
Stage the changes and then commit.
...
There are many options, but we (the teachers) suggest:
...
2. **Pull frequently**
   ...
   **Daily or every time you start working** on your branch. To keep your branch up-to-date with others’ changes so merges don’t become massive or painful.
   ...
   If more than one person is working on the same branch, you also need to pull from the feature branch.
